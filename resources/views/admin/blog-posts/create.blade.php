...
@section('breadcrumbs', 'Blog / Artículos / Nuevo')
    ...
    <form action="{{ route('admin.blog-posts.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title') }}" class="w-full border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500" required>
            ...
            <div>
                ...
                <input type="text" name="author" id="author" value="{{ old('author', Auth::user()->name) }}" class="w-full border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500">
            </div>
        ...
        <div>
            ...
            <textarea name="excerpt" id="excerpt" rows="2" class="w-full border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Breve resumen del artículo...">{{ old('excerpt') }}</textarea>
            @error('excerpt') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
        </div>

        <div>
            <label for="body" class="block text-sm font-medium text-gray-700 mb-1">Contenido</label>
            <textarea name="body" id="body" rows="20" class="w-full border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500" required>{{ old('body') }}</textarea>
