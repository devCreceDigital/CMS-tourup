...
@section('breadcrumbs', 'Programas / Editar')
    ...
    <div class="max-w-2xl">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200">
            <div class="px-6 py-5 border-b border-gray-200">
                <h2 class="text-lg font-semibold text-gray-900">Editar Programa: {{ $program->name }}</h2>
            </div>

            <form action="{{ route('admin.programs.update', $program->id) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="trip_category_id" id="trip_category_id"
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}" {{ old('trip_category_id', $program->trip_category_id) == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4"
                        ...
                        placeholder="Descripción del programa">{{ old('description', $program->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="duration_days" id="duration_days" value="{{ old('duration_days', $program->duration_days) }}" min="1"
                    ...
                    <div>
                        ...
                        <input type="text" name="age_range" id="age_range" value="{{ old('age_range', $program->age_range) }}"
                ...
                <div>
                    ...
                    <input type="text" name="image" id="image" value="{{ old('image', $program->image) }}"
